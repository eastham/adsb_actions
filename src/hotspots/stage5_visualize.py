...
def _airport_quality_js(airport_quality: dict | None,
    ...
    return (
        ...
        '             +   (explain[p.score] || "") + "</div>";\n'
        '    html += "<div><b>Total low-altitude tracks:</b> " + (p.totalLowAltTracks || 0) + "</div>";\n'
        '    if (p.score !== "none") {\n'
        '      html += "<div><b>Approach/departure completeness:</b> " + pct(p.completionRate) + "</div>"\n'
        ...
        '      html += "<div style=\\"margin-top:8px\\"><b>Approaches by runway:</b> "\n'
        '            + (parts.length ? parts.join(", ") : "No data.") + "</div>";\n'
        '    }\n'
...
def generate_html(df: pd.DataFrame, center_lat: float, center_lon: float,
    ...
    html += (
        ...
        '      "heatmap-weight": 1,\n'
        '      "heatmap-intensity": 1,\n'
        '      // Kernel doubles every 2 zoom levels (exp base sqrt(2)) so each\n'
        '      // event covers a roughly constant *ground* footprint rather than\n'
        '      // growing with screen size, which kept smearing a whole airport.\n'
        '      "heatmap-radius": ["interpolate", ["exponential", 1.414], ["zoom"], 8, 12, 10, 24, 12, 48, 14, 96],\n'
        '      "heatmap-opacity": 0\n'
        '    }\n'
        '  });\n'
        '\n'
...
def generate_pmtiles_html(pmtiles_path: str, sidecar_dir: str,
    ...
    html += (
        ...
        '    source: "events", "source-layer": "events",\n'
        '    paint: {"circle-color": "rgba(0,0,0,0)", "circle-radius": 8, "circle-stroke-width": 0}\n'
        '  });\n'
        '\n'
        '  map.on("mouseenter", "events-hit", function() {\n'
        '    map.getCanvas().style.cursor = "pointer";\n'
