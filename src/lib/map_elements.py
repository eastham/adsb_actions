...
def build_hide_script(points_json, qualities_json, map_name,
    ...
    return (
        ...
        "            // Honor the user-controlled slider if present; otherwise default to 0.\n"
        "            var opacitySlider = document.getElementById('heatmap-opacity-slider');\n"
        "            nativeHeatmapLayer._canvas.style.opacity = opacitySlider ? (opacitySlider.value / 100.0) : 0;\n"
        "        }\n"
        ...
        "    function setPanelOpacity(opacity, pointerEvents) {\n"
        "        panelIds.forEach(function(id) {\n"
        "            var el = document.getElementById(id);\n"
        "            if (el) { el.style.opacity = opacity; el.style.transition = 'opacity 0.2s'; el.style.pointerEvents = pointerEvents; }\n"
        "        });\n"
        "    }\n"
        "    var map = getMap();\n"
        "    if (map) {\n"
        "        map.on('popupopen', function() { setPanelOpacity('0.15', 'none'); });\n"
        "        map.on('popupclose', function() { setPanelOpacity('1.0', 'auto'); });\n"
        "    }\n"
        "});\n"
        "</script>\n"
    )
